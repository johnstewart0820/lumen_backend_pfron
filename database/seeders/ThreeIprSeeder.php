...
class ThreeIprSeeder extends Seeder
{
    /**
    ...
    public function run()
    {
        $collection = (new FastExcel)->import(storage_path('/app/iprs/3/iprs.xlsx'));
        ...
        for ($i = 0; $i < 5; $i ++ ){
            $collection_schedule = (new FastExcel)->import(storage_path('/app/iprs/3/'.$arr[$i]));
            foreach($collection_schedule as $item) {
                ...
                $id_candidate = $candidate_list[0]->id_candidate;
                $rehabitation_center_quaters = RehabitationCenterQuater::where('center_id', '=', 3)->get();
                $id_ipr= Ipr::where('id_candidate', '=', $id_candidate)->where('ipr_type', '=', 3)->first()->id;
                for ($j = 1; $j <= 42; $j ++) {
                    if (!isset($item[strval($j)]))
                        continue;
